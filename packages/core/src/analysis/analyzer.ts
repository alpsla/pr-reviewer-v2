...
export class CodeAnalyzer {
  ...
  private calculateComplexity(code: string): number {
    const branchingStatements = (code.match(/if|while|for|switch/g) || [])
      .length;
    ...
    lines.forEach((line, index) => {
      if (line.includes("document.querySelectorAll")) {
        issues.push({
          ...
          location: {
            line: index + 1,
            column: line.indexOf("document.querySelectorAll") + 1,
          },
        });
      }
    });
  ...
  private generateOptimizations(code: string): string[] {
    ...
    if (code.includes("for (")) {
      optimizations.push(
        "Consider using array methods like map/reduce for better performance",
      );
    }
