...
describe("EmailAuthService", () => {
  ...
  beforeEach(() => {
    ...
    emailAuthService = new EmailAuthService(mockSupabase, mockDb, mockConfig);

    mockDb.createUser.mockResolvedValue(mockBaseUser);
    mockDb.updateUser.mockResolvedValue(mockBaseUser);

    (mockSupabase.auth.verifyOtp as jest.Mock).mockResolvedValue({
      data: { session: mockSession, user: mockBaseUser },
      error: null,
    });
  });
  ...
  describe("sendMagicLink", () => {
    it("should send magic link successfully", async () => {
      ...
      await expect(
        emailAuthService.sendMagicLink(email),
    ...
    it("should throw when magic link sending fails", async () => {
      ...
      await expect(emailAuthService.sendMagicLink(email)).rejects.toThrow(
  ...
  describe("verifyMagicLink", () => {
    ...
    it("should throw when token verification fails", async () => {
      ...
      await expect(emailAuthService.verifyMagicLink(mockToken)).rejects.toThrow(
  ...
  describe("refreshToken", () => {
    ...
    it("should throw when token refresh fails", async () => {
      ...
      await expect(
        emailAuthService.refreshToken(mockRefreshToken),
  ...
  describe("signOut", () => {
    ...
    it("should sign out successfully", async () => {
      ...
      await expect(
        emailAuthService.signOut(mockSessionId),
      ).resolves.not.toThrow();
    ...
    it("should throw when sign out fails", async () => {
      ...
      await expect(emailAuthService.signOut(mockSessionId)).rejects.toThrow(
        "Failed to sign out",
      );
    });
